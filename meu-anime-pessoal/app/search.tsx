      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Image source={{ uri: item.images.jpg.image_url }} className="w-16 h-24 rounded-md" />
              <View className="ml-4 flex-1">
                <Text className="text-lg font-bold text-gray-800" numberOfLines={2}>{item.title}</Text>
                <Text className="text-gray-500">{item.year || 'Ano desconhecido'} • {item.episodes || '?'} eps</Text>
              </View>
